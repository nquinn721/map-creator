app.factory('FileManager', ['$http', '$rootScope', 'util', 'File',  function($http, $rootScope, util, File) {
	function FileManager(stage) {
		this.stage = stage;
	}
	...
	FileManager.prototype = {
		createFile : function(id, type, obj) {
			...
			this.stage.files[id] = new File(this.stage, {
				name : id, 
		...
		loadTileMap : function(map) {
			$http.get('/load-tilemap/' + map.id).then(function(data) {
				this.createFile(map.id, 'tilemap', data.data);
			}.bind(this));
		},
